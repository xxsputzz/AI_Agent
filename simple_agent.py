# Simple version of AI Agent with minimal dependencies
import os
import json
import requests
...
{context}
Question: {query}"""        # Check if Ollama is available by trying to connect        try:
        ...
        if ollama_available:
            print("Ollama is available, sending request...")
            try:
                # Try to use Ollama - use the correct endpoint
                response = requests.post(
                    "http://localhost:11434/api/generate",
                    json={
                        "model": "llama2",  # Try a different model that might be installed
                        "prompt": prompt,
                        "stream": False
                    },
